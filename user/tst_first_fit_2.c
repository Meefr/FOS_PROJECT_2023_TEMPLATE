 ...
 */
uint32 allocSizes[numOfAllocs] = { 3 * sizeof(int) + sizeOfMetaData(), 2
		* sizeOfMetaData(), 20 * sizeof(char) + sizeOfMetaData(), kilo / 2, 1
		* kilo, 3 * kilo / 2, 2 * kilo };
...
void _main(void) {
	...
	cprintf(
			"PREREQUISITE#1: Try to allocate set of blocks with different sizes [all should fit]\n\n");
	{
		is_correct = 1;
			...
			for (int j = 0; j < allocCntPerSize; ++j) {
				...
				if (va == NULL || (va < curVA)) {
					if (is_correct) {
						is_correct = 0;
						panic(
								"malloc() #1.%d: WRONG ALLOC - alloc_block_FF return wrong address. Expected %x, Actual %x\n",
	...
	cprintf(
			"1: Free set of blocks with different sizes (first block of each size)\n\n");
	{
		for (int i = 0; i < numOfAllocs; ++i) {
...
#define numOfFFTests 3
...

	{
		...
		uint32 expectedVAs[numOfFFTests] = { startOf4thFreeBlock,
				startOf1stFreeBlock, startOf4thFreeBlock + testSizes[0] };
		for (int i = 0; i < numOfFFTests; ++i) {
			...
			if (tstStartVAs[i] == NULL
				...
				is_correct = 0;
				cprintf(
						"malloc() #2.%d: WRONG FF ALLOC - alloc_block_FF return wrong address. Expected %x, Actual %x\n",
		...
		if (get_block_size(tstStartVAs[1]) != allocSizes[0]) {
			is_correct = 0;
			cprintf(
					"malloc() #3: WRONG FF ALLOC - make sure if the remaining free space doesn’t fit a dynamic allocator block, then this area should be added to the allocated area and counted as internal fragmentation\n");
			//break;
		}
		if (is_correct) {
			eval += 30;
			...
			"3: Try to allocate a block with equal to the first existing free block\n\n");
	{
		...
		//Check returned va
		void* expected = (void*) (startOf4thFreeBlock + testSizes[0]
				+ testSizes[2]);
		if (va == NULL || (va != expected)) {
			is_correct = 0;
			cprintf(
					"malloc() #4: WRONG FF ALLOC - alloc_block_FF return wrong address.expected %x, actual %x\n",
					expected, va);
		}
	...
	cprintf("4: Check stored data inside each allocated block\n\n");
	{
		...
		for (int i = 0; i <= numOfFFTests; ++i) {
			...
			if (*(tstStartVAs[i]) != (353 + i) || *(tstMidVAs[i]) != (353 + i)
					|| *(tstEndVAs[i]) != (353 + i)) {
				is_correct = 0;
				cprintf(
						"malloc #5.%d: WRONG! content of the block is not correct. Expected=%d, val1=%d, val2=%d, val3=%d\n",
						i, (353 + i), *(tstStartVAs[i]), *(tstMidVAs[i]),
						*(tstEndVAs[i]));
				break;
			}
		}
	...
	cprintf("5: Test a Non-Granted Request\n\n");
	{
		...
		uint32 numOfRem2KBAllocs = ((USER_HEAP_START + DYN_ALLOC_MAX_SIZE
				- (uint32) sbrk(0)) / PAGE_SIZE) * 2;
...
//			cprintf("i: %d\n", i);
			if (va == NULL) {
				is_correct = 0;
				cprintf(
						"malloc() #6.%d: WRONG FF ALLOC - alloc_block_FF return NULL address while it's expected to return correct one.\n");
				break;
			}
		}
		...
		if (va != NULL) {
			is_correct = 0;
			cprintf(
					"malloc() #7: WRONG FF ALLOC - alloc_block_FF return an address while it's expected to return NULL since it reaches the hard limit.\n");
		}
		if (is_correct) {
			eval += 20;
		}
	...
	cprintf(
			"test FIRST FIT (2) [DYNAMIC ALLOCATOR] is finished. Evaluation = %d%\n",
			eval);
