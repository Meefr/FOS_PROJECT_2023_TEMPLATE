...
#if USE_KHEAP
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e,
			...
			if (ROUNDDOWN(ptr_WS_element->virtual_address,
				...
				if (ptr_tmp_WS_element != NULL) {
					...
					pt_set_page_permissions(e->env_page_directory,
							ptr_tmp_WS_element->virtual_address, PERM_PRESENT,
							0);
				}
				found = 1;
				break;
...
void env_page_ws_print(struct Env *e) {
	if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
		...
		LIST_FOREACH(wse, &(e->page_WS_list))
		{
			...
			uint32 perm = pt_get_page_permissions(e->env_page_directory,
					virtual_address);
...
void env_table_ws_print(struct Env *e) {
	...
	for (i = 0; i < __TWS_MAX_SIZE; i++) {
		...
		cprintf(", used bit = %d, time stamp = %d",
				pd_is_table_used(e->env_page_directory, virtual_address),
				e->__ptr_tws[i].time_stamp);
		if (i == e->table_last_WS_index) {
			cprintf(" <--");
		}
		cprintf("\n");
